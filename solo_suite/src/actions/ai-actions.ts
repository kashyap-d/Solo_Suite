...
export async function debugGenerateTasks(
  ...
  try {
    ...
    logs.push("✅ Groq API call successful for task generation!")

    const responseText = completion.choices[0]?.message?.content || ""
    logs.push(`📝 Raw response text: ${responseText.substring(0, 500)}...`)

    if (!responseText) {
      logs.push("❌ Empty response from Groq API")
      throw new Error("Empty response from Groq API")
    }

    // Clean the response to ensure it's valid JSON
    let cleanedText = responseText.trim()
    logs.push(`🧹 Text before cleaning: ${cleanedText.substring(0, 200)}...`)

    // Remove any markdown code blocks if present
    if (cleanedText.startsWith("```json")) {
      cleanedText = cleanedText.replace(/```json\n?/, "").replace(/\n?```$/, "")
      logs.push("🧹 Removed ```json blocks")
    } else if (cleanedText.startsWith("```")) {
      cleanedText = cleanedText.replace(/```\n?/, "").replace(/\n?```$/, "")
      logs.push("🧹 Removed ``` blocks")
    }

    logs.push(`🧹 Cleaned text: ${cleanedText}`)

    let tasks
    try {
      tasks = JSON.parse(cleanedText)
      logs.push("✅ JSON parsing successful!")
      logs.push(`📋 Parsed tasks: ${JSON.stringify(tasks, null, 2)}`)
    } catch (parseError) {
      logs.push(`❌ JSON Parse Error: ${parseError}`)
      logs.push(`🔍 Raw response that failed to parse: ${responseText}`)
      logs.push(`🔍 Cleaned text that failed to parse: ${cleanedText}`)
      throw new Error(`Failed to parse JSON response: ${parseError}`)
    }

    // Validate the tasks array
    if (!Array.isArray(tasks)) {
      logs.push(`❌ Response is not an array: ${typeof tasks}`)
      throw new Error("Response is not an array")
    }

    logs.push(`✅ Validation passed - tasks is an array with ${tasks.length} items`)

    // Validate each task has required fields
    const validatedTasks = tasks.map((task: any, index: number) => {
      logs.push(`🔍 Validating task ${index + 1}: ${JSON.stringify(task)}`)

      if (!task.title || !task.description || !task.priority || typeof task.estimatedHours !== "number") {
        logs.push(`❌ Invalid task structure at index ${index}: ${JSON.stringify(task)}`)
    ...
    logs.push("💾 Saving to Supabase...")
    logs.push(`📋 Tasks to insert: ${JSON.stringify(validatedTasks, null, 2)}`)

    const { data, error } = await supabase.from("tasks").insert(validatedTasks).select()

    if (error) {
      logs.push(`❌ Supabase error: ${JSON.stringify(error, null, 2)}`)
      throw error
    }

    logs.push(`✅ Successfully saved ${data?.length} tasks to database`)
    logs.push(`📋 Saved tasks: ${JSON.stringify(data, null, 2)}`)
    ...
    logs.push(`❌ Error in generateTasks: ${error.message}`)
...
export async function generateTasks(
  ...
  try {
    console.log("📡 Making Groq API call for task generation...")
    ...
    console.log("✅ Groq API call successful for task generation!")
  ...
  try {
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "user",
          content: 'Say \'Hello, Groq is working!\' in JSON format: {"message": "your response"}',
        },
      ],
      model: "llama3-70b-8192",
      temperature: 0.1,
    ...
    return {
      success: false,
      message: `Groq test failed: ${error.message}`,
      error: error,
    }
  }
}
